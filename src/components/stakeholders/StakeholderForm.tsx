...
import { WarningCircle, Plus, TrashSimple } from "@phosphor-icons/react";
...
interface StakeholderFormProps {
  onSuccess: (stakeholderId?: number) => void;
  onCancel: () => void;
  showProcessWarningLink?: boolean;
  processWarningLinkUrl?: string;
}
...
export default function StakeholderForm({
  ...
  processWarningLinkUrl = "/ops/stakeholders",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newStakeholderId = await createStakeholder({
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-warning/10 border-2 border-warning/30 dark:bg-warning/20 rounded-lg p-6 sm:p-8 text-center">
          ...
          <p className="text-sm sm:text-base text-foreground-secondary mb-4 sm:mb-6">
            ...
            {!showProcessWarningLink && " Please contact your administrator to set up stakeholder processes."}
          </p>
          <button
            onClick={() => onCancel()}
          ...
          >
            {showProcessWarningLink ? "Go to Process Management" : "Back to Stakeholders"}
          </button>
        </div>
      </div>
    );
