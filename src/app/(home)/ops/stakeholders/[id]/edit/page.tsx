...
export default function EditOpsStakeholderPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const { id } = use(params);
  const stakeholderId = parseInt(id);
  const { employeeInfo, canWrite } = useAuth();
  ...
  // Check if user can edit this stakeholder
  const canEdit = () => {
  ...
  useEffect(() => {
    ...
    loadData();
  }, [stakeholderId, fetchProcesses, fetchEmployees, fetchStakeholderById, fetchStakeholders]);

  // Redirect if user doesn't have permission after stakeholder loads
  useEffect(() => {
    if (!loadingStakeholder && stakeholder && !canEdit()) {
      toast.error("You don't have permission to edit this stakeholder");
      router.push(`/ops/stakeholders/${stakeholderId}`);
    }
  }, [loadingStakeholder, stakeholder, stakeholderId, router]);
  ...
  if (!canEdit()) {
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="max-w-2xl mx-auto text-center py-12">
          <WarningCircle className="mx-auto text-warning" size={48} />
          <h2 className="text-xl font-bold text-foreground-primary mt-4">Permission Denied</h2>
          <p className="text-foreground-secondary mt-2">
            You don't have permission to edit this stakeholder. Only the assigned KAM or users with stakeholder write permissions can edit.
          </p>
          <button
            onClick={() => router.push(`/ops/stakeholders/${stakeholderId}`)}
            className="mt-6 px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700"
          >
            View Stakeholder
          </button>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-warning/10 border-2 border-warning/30 dark:bg-warning/20 rounded-lg p-8 text-center">
            ...
            <p className="text-foreground-secondary mb-6">
              Contact your administrator to set up stakeholder processes.
            </p>
            <button
              onClick={() => router.push(`/ops/stakeholders/${stakeholderId}`)}
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-3 mb-6">
          <h1 className="text-2xl font-bold text-foreground-primary">Edit Stakeholder</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-surface-primary rounded-lg border border-border-primary p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2 border rounded-lg bg-surface-primary text-foreground-primary focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none ${
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 border border-border-secondary rounded-lg bg-surface-primary text-foreground-primary focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
            ...
            {/* Only show KAM field if user has write permissions (admin) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 text-sm border border-border-secondary rounded bg-surface-primary text-foreground-primary focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={`w-full px-3 py-2 text-sm border rounded bg-surface-primary text-foreground-primary focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none ${
          ...
          <div className="flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={() => router.back()}
              className="px-6 py-2 border border-border-secondary text-foreground-secondary rounded-lg hover:bg-surface-hover"
              disabled={submitting}
            >
              Cancel
            </button>
